...
// Record trace - Record address trace for debugging or other purposes
    ...
    assert(ok);
    if (ok) {
        assert(trans.shard_id!=-1 && trans.node_id!=-1 && trans.in_node_id!=-1);
        ctrls_[channel]->AddTransaction(trans);
    }
    last_req_clk_ = clk_;
    return ok;
...
std::vector<Transaction> JedecDRAMSystem::GCNearClockTick()
{
    std::vector<Transaction> finished_trans;
    for (size_t i = 0; i < ctrls_.size(); i++) 
    {
        // look ahead and return earlier
        while (true) 
        {
            Transaction trans = ctrls_[i]->ReturnDoneWholeTrans(clk_);
            if(trans.addr==-1)
                break;
            
            finished_trans.push_back(trans);
            if (trans.is_write) 
